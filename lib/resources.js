...
function ShakerResources(context, logger) {
    'use strict';
    this.logger = logger;
...
ShakerResources.prototype = {
        ...
        function getDependencies(rawResource) {
            ...
            Y.Array.each(module.requires, function (requiredModule) {
                ...
                if (yuiResources[requiredModule]) {
                    ...
                    //console.log('list - ' + requiredModule)
                ...
                Y.Array.each(rawDependencies, function (rawDependency) {
                    var dependency;
                    //console.log(rawDependency.name)
                    if (!rawDependency) {
                        return;
                    }
    ...
    _makeStore: function (cfg) {
        'use strict';
        var YUIFactory = require('mojito/lib/yui-sandbox.js'),
            originalGetYUI = YUIFactory.getYUI,
            libstore,
            store;
    ...
    _getShakerConfig: function (context) {
        ...
        if (shakerConfig.resources) {
            ...
            if (shakerConfig.resources.yui === true) {
                // remove yui option such that it is replaced by defaults
                delete shakerConfig.resources.yui;
            } else if (Y.Lang.isArray(shakerConfig.resources.yui)) {
                // add default modules
                Y.Array.each(DEFAULT_RESOURCES.yui, function (module) {
                    var index = shakerConfig.resources.yui.indexOf(module);
                    // remove duplicate
                    if (index !== -1) {
                        shakerConfig.resources.yui.splice(index, 1);
                    }
                });
                Array.prototype.unshift.apply(shakerConfig.resources.yui, DEFAULT_RESOURCES.yui);
            }
            // remove mojito option such that it is replaced by defaults
            if (shakerConfig.resources.yui === true) {
                delete shakerConfig.resources.mojito;
            } else if (Y.Lang.isArray(shakerConfig.resources.mojito)) {
                // add default modules
                Y.Array.each(DEFAULT_RESOURCES.mojito, function (module) {
                    if (shakerConfig.resources.mojito.indexOf(module) === -1) {
                        shakerConfig.resources.mojito.unshift(module);
                    }
                });
            }
    ...
    _shakeMojito: function () {
        ...
        Y.Array.each(self.shakerConfig.resources.mojito, function (mojitoModule) {
    ...
    _shakeYUI: function () {
        ...
        Y.Array.each(self.shakerConfig.resources.yui, function (yuiModule) {
            var rawResource,
                resource;

            if (self._yuiResources[yuiModule]) {
                rawResource = self._yuiResources[yuiModule].getRawResource();
            }
            if (rawResource) {
                resource = self._storeYUIModule(rawResource, {});
    ...
    _getYUIResources: function () {
        'use strict';
        var self = this,
            yuiPath = process.cwd() + '/node_modules/',
