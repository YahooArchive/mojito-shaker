...
/* LOGGING FUNCTION */
ShakerCore.prototype._log = function (f, err) { if (ShakerCore.DEBUG) {console.log(f + ': ' + err);}};
...
ShakerCore.prototype._getMojits = function (app_config) {
    ...
    for (i = 0; i < mojitFolders.length; i++) {
        ...
        } catch (error) {            this._log('_getMojits' + error);
        }
...
ShakerCore.prototype._generateClientSideResources = function (mojit, action, resources, modules) {
    ...
    clientDependencies.dependencies = removeDuplicates(clientDependencies.dependencies);
