...
ShakerCore.prototype.filterResourceVersionsByContext = function (ctx, ress) {
...
ShakerCore.prototype.getAppResourcesByContext = function (context) {
    var rawResources = this.store.getResourceVersions({mojit:'shared'}),
    ...
    return this.filterResourceVersionsByContext(context, rawResources);
};

ShakerCore.prototype.getMojitResourcesByContext = function (mojitName, ctx) {
    var mojitResources = this.store.getResourceVersions({mojit: mojitName}),
        ...
        sharedResources = this.store.getResourceVersions({mojit: 'shared'}),
    ...
    mojitResources = this.filterResourceVersionsByContext(ctx, mojitResources);
    sharedResources = sharedResources.filter(sharedResourcesFilterIterator);
    sharedResources = this.filterResourceVersionsByContext(ctx, sharedResources);
...
ShakerCore.prototype.getYUIDependenciesForApp = function (ctx) {
    var item,
        store = this.store,
        shared = store.yui.getConfigShared('client'),
...
ShakerCore.prototype.bundleShakenMojit = function (mojitName, context, shakenMojit) {
    var appShakerMerge = this.getMergedShakerConfigByContext('shared', context),
        ...
        // Attach the global inline assets collected on @shakenMojitByContext if any
        if (shakenMojit.inlineAssets.length) {
            bundleMojit.inlineBlob = shakenMojit.inlineAssets;
        }
        ...
        Y.Object.each(shakenMojit.views, function (viewResource, viewName) {
            ...
            bundleMojit[viewName].inlineBlob = shakenMojit.inlineAssets.concat(viewResource.inlineBlob);
...
ShakerCore.prototype.shakeMojitByContext = function (mojitName, context) {
        ...
        switch(resource.type) {
            ...
            case 'asset':
                if (resource.source.inline) {
                    mojitResources.inlineAssets.push(resource);
                    return;
                }
    ...
    // check if the controller exists (it may have server afinity)
    ...
    Y.Object.each(mojitResources.views, function (viewResource, viewName) {
         ...
         var actionAssets = mojitResources.assets.filter(function (item) {
            ...
            inlineAssetsAction = mojitResources.inlineAssets.filter(function (item) {
                return ((item.name.indexOf(viewName) === 0) || item.source.fs.basename.indexOf(viewName) === 0);
            });

         viewResource.css = actionAssets || [];
         viewResource.inlineBlob = inlineAssetsAction || [];

         assetsByAction = assetsByAction.concat(actionAssets);
         assetsInlineByAcion = assetsInlineByAcion.concat(inlineAssetsAction);
    });
    //we discard the ones are attached by action.
    mojitResources.css = libutils.arrayDiff(mojitResources.assets, assetsByAction);
    mojitResources.inlineAssets = libutils.arrayDiff(mojitResources.inlineAssets, assetsInlineByAcion);

    //console.log(mojitResources.views && mojitResources.views.index);
...
ShakerCore.prototype.shakeAppResourcesByContext = function (context) {
    var inlineFiles = {},
        appResources = this.getAppResourcesByContext(context),
        assets = appResources.filter(function (item) {
            return item.type === 'asset' && item.subtype === 'css';
        }),
        inline = appResources.filter(function (item) {
            return item.source.inline;
        }),
        name,
        res,
        pos,
        i;
    //we need to get the name of the shared inline resources for further mapping
    for (i in inline) {
        res = inline[i];
        //defensive check: efectively we have the selector in the name
        if (res.name.indexOf(INLINE_SELECTOR) !== -1) {
            pos = res.name.split('-');
            name = pos.shift();
            inlineFiles[name] = res;
        }
    }

    return {assets: assets, inlineShared: inlineFiles};
};
...
ShakerCore.prototype.shakeAllContexts = function () {
    var contextList = this._getUsedContexts(),
        ...
        shaken = {},
        originalContext = this.getStoreConfigs().context || {},
        originalEnv = originalContext.environment,
        mojitShaken,
        appShaken,
        env,
        mojit,
    ...
    for (i in contextList) {
        ...
        if (env === originalEnv && !shaken[stringContext]) {
            this.logger.debug('[SHAKER] - Analyzing context:' + stringContext, 4);
            for (j in mojits) {
                mojit = mojits[j];
            ...
            shaken[stringContext] = {
                mojits: mojitShaken,
                app: appShaken.assets,
                inlineShared: appShaken.inlineShared,
                routesBundle: {}
            };
            //if we have appBundle option we put there everything in there..
            if (shakerConfig.appBundle) {
                shaken[stringContext].appBundle = this.bundleAll(mojitShaken, appShaken.assets, shakerConfig);
            //else we check for routeBundle
            } else {
                shaken[stringContext].routesBundle = this.bundleByRoutes(mojitShaken, appShaken.assets, shakerConfig, context);
            }
        }
    }
    return shaken;
...
ShakerCore.prototype.shakeCore = function () {
    var context = this.getStoreConfigs().context,
        shakerConfig = this.getMergedShakerConfigByContext('shared', context),
        mojitoResources = this.store.getResourceVersions({mojit:'shared'});
    mojitoResources = mojitoResources.filter(function (item) {
        return (item.source.pkg.name === 'mojito' || item.source.pkg.name === 'mojito-shaker') &&
               (item.affinity.affinity === 'common' || item.affinity.affinity === 'client') &&
               item.source.fs.ext === '.js';
    });
    if (shakerConfig.filterInCore) {
        mojitoResources = mojitoResources.filter(function (item) {
            return (shakerConfig.filterInCore.indexOf(item.yui.name) === -1);
        });
    }
    return this.filterResourceVersionsByContext(context, mojitoResources);
};
