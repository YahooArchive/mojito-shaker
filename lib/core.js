...
ShakerCore.prototype._getMojits = function () {
    var store = this._store,
        mojits = util.simpleClone(store._mojitPaths),
        blackList = ['HTMLFrameMojit','ShakerHTMLFrameMojit','LazyLoad','DaliProxy'],
        item;
...
ShakerCore.prototype._shakeCore = function (resources) {
    ...
    core = files.filter(function (file) {
        //TODO: Refactor this with a regexp
        return (file.indexOf('/mojito/lib/app/') !== -1 || file.indexOf('/mojito/source/lib/app/') !== -1);
    }, this);
...
ShakerCore.prototype.getShakerConfig = function () {
    return (this.getConfig()).shaker || {};
};

ShakerCore.prototype.isMojitApp = function () {
    var app = this._store._appMeta,
        isApp = !util.isEmpty(app);
        isApp = isApp && app.server && app.server['*'] && app.server['*']['config-application'];
