...
ShakerCore.prototype._getMojits = function (app_config, paths) {
...
ShakerCore.prototype._recursiveModuleCalculation = function (item, modules) {
    ...
    if (modules[item]) {
        ...
        for (i in req) {
            if (modules[req[i]]) {
                dependencies = dependencies.concat(this._recursiveModuleCalculation(req[i], modules));
...
ShakerCore.prototype._generateClientSideResources = function (mojit, action, resources, modules) {
    mojit = mojit || {};
    ...
    var i, fileparts,
        ...
        clientDependencies = {models: [], controllers: [], dependencies: [], binders: [], views: []};
    ...
    for (i in models) {
        ...
        if (fileparts[1] !== 'server') {
            ...
            clientDependencies.dependencies = clientDependencies.dependencies.concat(this._calculateBinderDependencies(action,models[i],modules));
        }
    }
...
ShakerCore.prototype._shakeMojit = function (name, path, options) {
    ...
    var actions, action, clientSideDependencies, dispatched, meta, listFiles,
        ...
        var mergedAutoload = options.app ? resources.autoload : util.simpleMerge(self._resources.app.autoload,resources.autoload),
        modules = self._precalculateAutoloads(mergedAutoload),
    ...
    for (action in (actions = shaker_config.actions)) {
        clientSideDependencies = options.app ? {} : self._generateClientSideResources(name, action, resources, modules);
...
ShakerCore.prototype._mojitResources = function (urls) {
    var url, filename, content_type, type, base, split,
        ...
        //mojits = this._store.listAllMojits('server'); //FOR SOME REASON STORE MOJITO doesnt work well...
        mojits = this._getMojits();

    mojits.forEach(function (mojit) {
        if(mojit !== 'DaliProxy' && mojit !== 'HTMLFrameMojit' && mojit !== 'LazyLoad'){
            resources.mojits[mojit] = {assets: {}, binders: {}, autoload: {}, views: {}};
        }
    });
...
ShakerCore.prototype.getConfig = function() {
    return this._app.shaker !== undefined ? this._app.shaker : {};
};
