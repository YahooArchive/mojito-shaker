...
ShakerCore.prototype.generateClientSideResources = function (mojit, action, resources, modules) {
    ...
    if (controller_affinity !== 'server') {
        ...
        clientDependencies.dependencies = clientDependencies.dependencies.concat(this.calculateBinderDependencies(action,controllerFile,modules));
    }
    ...
    for (i in models) {
        ...
        if (fileparts[1] !== 'server') {
            ...
            clientDependencies.dependencies = clientDependencies.dependencies.concat(this.calculateBinderDependencies(action,models[i],modules));
        }
    }
    ...
    for (i in binders) {
        ...
        if (action === fileparts[0]) {
            ...
            clientDependencies.dependencies = clientDependencies.dependencies.concat(this.calculateBinderDependencies(action, binders[i], modules));
        }
    }
...
ShakerCore.prototype.shakeImages = function (resources) {
    var image, images = [];
...
ShakerCore.prototype.run = function (options) {
    ...
    shaken.images = this.shakeImages(this._resources);
