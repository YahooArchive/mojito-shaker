...
ShakerCore.prototype.bundleByRoutes = function (shakenMojits, shakenApp, config) {
    ...
    Y.Object.each(routeBundle, function (bundle, routeName) {
        var bundleRoute = {js:[], css: shakenApp.slice(0), requires: {}};
        bundle.forEach(function (mojit) {
            ...
            if (shakenMojit) {
                ...
                bundleRoute.requires = Y.merge(bundleRoute.requires, shakenMojit.requires);
            }
        });
    ...
    for (i in contextList) {
        ...
        if (!shaken[stringContext]) {
            ...
            //else we check for routeBundle
                ...
                shaken[stringContext].routesBundle = this.bundleByRoutes(mojitShaken, appShaken, shakerConfig);
            }
        }
    }
    return shaken;
