...
function Shaker(store) {
    ...
    var config = this._store.getAppConfig(null, 'definition') || {},
        ...
        shaker = config.shaker !== undefined ? config.shaker : {};
    ...
    this._app = this._store._shortRoot;
    if (typeof appConfig.appName !== 'undefined') {
        this._app = appConfig.appName;
    }
    ...
    this._config.root = this._prefix + '/' + this._app + '/'; // Static prefix for assets generated
}
        ...
        for (action in metadata.app) {
            for (dim in metadata.app[action].shaken) {
                for (item in (list = metadata.app[action].shaken[dim])) {
                    renamed = list[item].replace(this._store._root, this._prefix + '/' + this._store._shortRoot);
                    renamed = renamed.replace(this._prefix + '/' + this._store._shortRoot + '/mojits', this._prefix);
                    list[item] = renamed;
                }
            }
        }
