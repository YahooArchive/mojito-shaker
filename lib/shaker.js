...
try {
    ...
    mojito = require('mojito/management/utils');
}
...
Image.prototype = {
    push: function (registry, options, callback) {
        var queue = new Queue(this._name, {registry: registry}),
            config = utils.simpleClone(options.config);
...
ClientRollup.prototype = {
    push: function (registry, options, callback) {
        ...
        queue.task('files', this._files);
        queue.task('concatcb', {
            callback: function (filename, content) {
...
function Shaker(core) {
    ...
    this._config.root = this._core.getPrefix() + '/' + this._core.getRoot() + '/'; // Static prefix for assets generated
}
...
Shaker.prototype = {
    ...
    run: function (callback) {
        ...
        var metadata = this._core.shakeAll();
            ...
            for (action in metadata.mojits[mojit]) {
                for (dim in metadata.mojits[mojit][action].shaken) {
                    for (item in metadata.mojits[mojit][action].shaken[dim]) {
                        ...
                        list[item] = list[item].replace(this._store._root + '/mojits', this._core.getPrefix());
                    }
                }
            }
        }
        ...
        for (action in metadata.app) {
            for (dim in metadata.app[action].shaken) {
                for (item in (list = metadata.app[action].shaken[dim])) {
                    renamed = list[item].replace(this._store._root, this._core.getPrefix() + '/' + this._store._shortRoot);
                    renamed = renamed.replace(this._core.getPrefix() + '/' + this._store._shortRoot + '/mojits', this._core.getPrefix());
                    list[item] = renamed;
                }
            }
        }
    ...
    _queueRollups: function (queue, metadata) {
        ...
        // Mojito core assets
        queue.push({
            object: new Rollup(Shaker.COMPILED_DIR + 'core_{checksum}.js', metadata.core.slice() /* Clone */),
            files: metadata.core
        });
        ...
        for (mojit in metadata.mojits) {
            for (action in metadata.mojits[mojit]) {
                ...
                if (client.length) {
                    queue.push({
                        object: new ClientRollup(Shaker.COMPILED_DIR + 'client_' + mojit + '_{checksum}.js', this._store._root, client.slice() /* Clone */),
                        files: client
                    });
                    client.length = 0;
                }
                ...
                for (dim in (files = metadata.mojits[mojit][action].shaken)) {
                    if (files[dim].length) {
                        ...
                        if (filtered.js.length) {
                            queue.push({
                                object: new Rollup(Shaker.COMPILED_DIR + name + '.js', filtered.js),
                                files: files[dim]
                            });
                        }
                        if (filtered.css.length) {
                            queue.push({
                                object: new Rollup(Shaker.COMPILED_DIR + name + '.css', filtered.css),
                                files: files[dim]
                            });
                        }
                        files[dim].length = 0;
        ...
        for (action in metadata.app) {
            ...
            if (client.length) {
                queue.push({
                    object: new ClientRollup(Shaker.COMPILED_DIR + 'appclient_' + action + '_{checksum}.js', this._store._root, client.slice() /* Clone */ ),
                    files: client
                });
                client.length = 0;
            }
    ...
    _writeMeta: function (metadata) {
        ...
        fs.writeFileSync(self._store._root + '/autoload/compiled/shaker-meta.common.js', content);
    }
};
