...
Image.prototype = {
    push: function (gearbox, options, callback) {
        var config = utils.simpleClone(options.config),
            queue = gear.queue({gearbox: gearbox})
                .load(this._file);
        ...
        queue.task(options.task, config)
            .run(function(err, results) {
                if (err) {
                    logger.error('[SHAKER] - Failed to push file ' + config.file + ' (' + err + ')');
                    callback(err);
                    return;
                }
                var result = results[0];
                callback(null, result.properties.url ? result.properties.url : config.root + result.properties.name);
            });
    }
};
            ...
            queue = gear.queue({gearbox: gearbox})
                ...
                .concat({
                    callback: function (blob) {
                        var filename = blob.properties.name,
...
function Shaker (options) {
    ...
    this._gearbox = gear.gearbox({dirname: path.resolve(__dirname, '../', 'node_modules', 'gear-lib', 'lib')});

    if (path.existsSync(Shaker.TASKS_DIR)) {    // Load more tasks
        this._gearbox.load({dirname: Shaker.TASKS_DIR});
    }
}
...
Shaker.prototype = {
    ...
    _lintFiles: function (callback) {
        ...
        if (filtered.css.length > 0) {
            gear.queue({gearbox: this._gearbox})
                .load(filtered.css)
                .csslint({callback: function(name, errors) {
                    name = (name ? name : 'files...');
                    logger.log('[SHAKER] - Linting ' + name);
                    if (errors) {
                        logger.error(name + ' - ' + errors.length + ' CSSLint errors:');
    ...
    _compileRollups: function (metadata, callback) {
        ...
        queue = async.queue(function (item, taskCallback) {
            setTimeout(function () {
                ...
                item.object.push(self._gearbox, options, function (err, url) {
                    if (err) {
