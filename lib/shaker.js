...
Shaker.prototype = {
    ...
    */
    run: function (callback) {
        ...
        var self = this,
            ...
            comboCDN = shakerConfig.useComboCDN,
         ...
         //        self.writeMetadata(data, function (err, data) {
         //            if (!err && comboCDN) {
                        self.logger.log('[SHAKER] - Pushing assets for ComboLoading...');
                        self.processResources(shakerConfig, callback);
         //            } else {
         //                callback(err, data);
         //            }
         //        });
    ...
    */
    ...
    processResources: function (shakerConfig, callback) {
        var self = this,
            ...
            resources = (Y.Object.values(resourcesMap)).slice(0, 10);

        queue.readYUIResources(resources);
        queue.preprocess();
        // write the metadata naming the files automatically based on their filename
        // queue.task('mobstorauto', {
        //     truncatechecksum: -6,
        //     task: taskOptions,
        //     ext: '.js'
        // });
...
//  -------============= Gear Tasks ===============-------
    ...
    readResourcesTask: function (item, done) {
        ...
        this.core.store.getResourceContent(item, function (err, content, stat) {
            done(null, new gear.Blob(content.toString(), {name: filename, resource: item, test:[item.url]}));
        });
    ...
    readYuiResourcesTask: function (item, done) {
        item = item || {};
        var filename = item.yui.name + '_{checksum}.js';
        this.core.store.getResourceContent(item, function (err, content, stat) {
            done(null, new gear.Blob(content.toString(), {name: filename, resource: item, test:[item.url]}));
        });
