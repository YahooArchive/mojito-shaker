...
Image.prototype = {
    push: function (registry, options, callback) {
        ...
        queue.task(options.task, config, function(err, results) {
            callback(null, results[0].meta.url ? results[0].meta.url : config.root + results[0].meta.name);
        }).run();
    }
};
...
function Shaker (options) {
    ...
    this._registry = gear.registry({dirname: path.resolve(__dirname, '../', 'node_modules', 'gear-lib', 'lib')});

    if (path.exists(Shaker.TASKS_DIR)) {    // Load more tasks
