...
Image.prototype = {
    push: function (registry, options, callback) {
        var queue = gear.queue({registry: registry}),
            config = utils.simpleClone(options.config);

        queue.load([this._file]);
        queue.read(); // Read the files into strings for writing
...
Rollup.prototype = {
    push: function (registry, options, callback) {
        ...
        queue.task(options.task === 'local' ? 'write' : options.task, config, function(err, results) {
            callback(null, results[0].meta.file);
        }).run();
    }
};
        ...
        var queue = gear.queue({registry: registry})
            ...
            .concat({
                callback: function (message) {
                    var filename = message.meta.file;
...
function Shaker (options) {
    ...
    this._task = shaker.task || DEFAULT_TASK; // Task to run when compiling assets
    ...
    this._registry = gear.registry({module: 'gear-lib'});
...
Shaker.prototype = {
    ...
    _runTask: function(metadata, callback) {
        if (this._task && this._task !== DEFAULT_TASK) {
    ...
    _lintFiles: function (callback) {
        ...
        if (filtered.css.length > 0) {
            gear.queue({registry: this._registry})
                .load(filtered.css)
                .csslint({}, function(err, results) {
                    /*
                    if (results.meta.lint) {
                        logger.log(results.meta.lint);
                    }*/
                })
