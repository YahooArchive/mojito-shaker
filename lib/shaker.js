    ...
    processResources: function (shakerMeta, shakerConfig, callback) {
        ...
        queue.run(function (err, uploadedFiles) {
            ...
            Y.Array.each(uploadedFiles, function (itemResult) {
                //this nesting is really tricky
                //but I dont want to return specific data outside the blob
                shakerMeta.cdnModules[itemResult.blob.resource.url] = itemResult.result[0];
            });
            callback(undefined, shakerMeta);
        });


    },
    processMojitJS: function (jsResources, viewResources, shakerOptions, taskMeta, callback) {
        if (shakerOptions.comboCDN) {
            callback(undefined, []);
            return;
        }
