...
Shaker.prototype = {
    ...
    run: function (callback) {
        var self = this,
            ...
            shakerConfig = this.core.getMergedShakerConfigByContext('shared', masterContext),
    ...
    processMojitJS: function (jsResources, viewResources, shakerOptions, callback) {
        var queue = new gear.Queue({registry: this._registry}),
            ...
            taskOptions = Y.merge(shakerOptions.taskConfig || {}, Shaker.DEFAULT_TASK_WRITE_CONFIG),
            resourceList = [];
        queue.tasks({
...
//  -------============= Gear Tasks ===============-------
    ...
    writeRawResources:function (options, blob, done){
        ...
        .filter(function (url) {return path.extname(url) === '.js' || path.extname(url) === '.css';});
    ...
    writeRollupsLocally: function (options, blob, done) {
        var task = options.task,
            ...
            prefix = this.core.getStoreConfigs(),
            transformedUrl = path.join('/', prefix, baseName, task.compiledAssetsPath),
            fullPath = path.join(this.root, task.compiledAssetsPath, task.compiledName) + task.ext;
