...
function Shaker (options) {
    ...
    this._registry.load(Shaker.TASKS_DIR);
}
...
Shaker.prototype = {
    ...
    _lintFiles: function (callback) {
        logger.log('[SHAKER] - Linting files...');
        ...
        if (filtered.css.length > 0) {
            queue = new Queue('css lint queue', {registry: this._registry});
            
            queue.on('taskComplete', function (data) {
                if (data.task.type === 'csslint2') {
                    if (data.result === 'lint ok') {
                        callback();
                    } else {
                        logger.error('[SHAKER] - ' + data.result);
                    }
                }
            });

            queue.task('files', filtered.css);
            queue.task('csslint2');
            queue.run();
        }
    },
    ...
    _queueRollups: function (queue, metadata) {
        ...
        for (mojit in metadata.mojits) {
            for (action in metadata.mojits[mojit]) {
                ...
                if (client.length) {
                    queue.push({
                        object: new ClientRollup(Shaker.COMPILED_DIR + 'client_' + mojit + '_{checksum}.js', this._core.getAppRoot(), client.slice() /* Clone */),
                        files: client
                    });
                    client.length = 0;
                }
        ...
        for (action in metadata.app) {
            ...
            if (client.length) {
                queue.push({
                    object: new ClientRollup(Shaker.COMPILED_DIR + 'appclient_' + action + '_{checksum}.js', this._core.getAppRoot(), client.slice() /* Clone */ ),
                    files: client
                });
                client.length = 0;
            }
