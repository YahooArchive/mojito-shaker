...
Shaker.prototype = {
    ...
    run: function (callback) {
        ...
        this.logger.success('[SHAKER] - Application analyzed correctly');
            ...
            switch(task.type) {
                case 'mojit':
                    self.processMojit(task.mojitResources, shakerConfig, task, function (err, data) {
                        self.logger.success('[SHAKER] - Processed Mojit: ' + task.mojitName + ' for context: ' + task.contextKey, 1);
                ...
                case 'bundle':
                    self.processMojit(task.bundle, shakerConfig, task, function (err, data) {
                        self.logger.success('[SHAKER] - Processed Bundling for context: ' + task.contextKey, 1);
                ...
                case 'app':
                    self.processMojitCSS(task.appResources, shakerConfig, task, function (err, data) {
                        self.logger.success('[SHAKER] - Processed App assets for context: ' + task.contextKey, 1);
    ...
    processMojitJS: function (jsResources, viewResources, shakerOptions, taskMeta, callback) {
        ...
        queue.run(function (err, data) {
            if (err) {
                ...
                throw {name: "Shaker Error", message: 'Building proccess failed'};
            } else {
