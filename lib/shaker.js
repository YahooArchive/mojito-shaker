...
Shaker.prototype = {
    ...
    _rename: function (metadata) {
        ...
        var mojit, action, dim, item, list, aux;
        ...
        for (action in metadata.app) {
            for (dim in metadata.app[action].shaken) {
                for (item in (list = metadata.app[action].shaken[dim])) {
                    aux = list[item].replace(this._store._root, this._prefix + '/' + this._store._shortRoot);
    ...
    _queueRollups: function (queue, metadata) {
        ...
        // Mojito core assets
        queue.push({
            object: new Rollup(Shaker.COMPILED_DIR + 'core_{checksum}.common.js', metadata.core.slice() /* Clone */),
            files: metadata.core
        });
        ...
        for (mojit in metadata.mojits) {
            for (action in metadata.mojits[mojit]) {
                ...
                if (client.length) {
                    queue.push({
                        object: new ClientRollup(Shaker.COMPILED_DIR + mojit + '_{checksum}.client.js', this._store._root, client.slice() /* Clone */),
                        files: client
                    });
                    client.length = 0;
                }
                ...
                for (dim in (files = metadata.mojits[mojit][action].shaken)) {
                    if (files[dim].length) {
                        ...
                        if (filtered.js.length) {
                            queue.push({
                                object: new Rollup(Shaker.COMPILED_DIR + name + '.common.js', filtered.js),
                                files: files[dim]
                            });
                        }
        ...
        for (action in metadata.app) {
            ...
            if (client.length) {
                queue.push({
                    object: new ClientRollup(Shaker.COMPILED_DIR + 'app_' + action + '_{checksum}.client.js', this._store._root, client.slice() /* Clone */ ),
                    files: client
                });
                client.length = 0;
            }
