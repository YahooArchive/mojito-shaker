...
Shaker.prototype = {
    ...
    _lintFiles: function (callback) {
        ...
        if (filtered.css.length > 0) {
            queue = gear.queue({registry: this._registry});
            
            queue.load(filtered.css);
            queue.csslint(null, function(err, result) {
                if (err) {
                    logger.error('[SHAKER] - ' + result.meta);
                }
            });
