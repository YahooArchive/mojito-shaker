...
console.log(Y.Object.isEmpty(config.locations));
        // only apply tasks if there is a location
        var gearTasks = !Y.Object.isEmpty(config.locations) ? configToGearTasks(config) : {};
    ...
    function applyRollups(config, appResources, organizedResources, done) {
        ...
        // TODO: should only be one route rollup
        // only apply rollup if there is a location
    ...
    function applyLocations(locationsConfig, appConfig, resources, done) {
        ...
        locationArray = Object.keys(locationsConfig)
