...
Y.extend(ShakerCompiler, Y.Base, {
    ...
    compile: function (done) {
        ...
        // load the addons if any in the resource store
        this._loadAddons(config);
    ...
    _loadAddons: function (config) {
        'use strict';
        var addons = {},
            shakerAddons = this.resources.store.getResourceVersions({
                type: 'addon',
                subtype: 'shaker'
            });
        ...
        Y.Object.each(Y.mojito && Y.mojito.addons && Y.mojito.addons.shaker, function (fn, name) {
            ...
            this.plug(fn, Y.merge(config && config.addons && config.addons.compiler && config.addons.compiler[name]));
        }, this);
    },
    ...
    preprocessRollupResources: function (resources) {
        'use strict';
        return resources;
    },
    ...
    _applyRollups: function (config, resources, modules, logger, done) {
        ...
        if (config.routeRollups) {
            ...
            try {
                rollups = modules.rollups[config.routeRollups.module](config.routeRollups, this.preprocessRollupResources(organizedResources));
            } catch (e) {
            ...
            Y.Object.each(rollups && rollups[posl], function (routeRollups, route) {
                ...
                Y.Object.each(routeRollups, function (rollup, type) {
                    ...
                    Y.Object.each(rollup.resources, function (resource, url) {
                        belongsInPOSL |= resource.selector === majorSelector;
                    });
                    if (belongsInPOSL) {
