...
function concatcbFilesSync(callback, destfile, sourcefiles, encoding) {
    ...
    try {
        ...
        throw new Error('Could not read the files to be concatenated : ' + sourcefiles.toString() + ' ' + e1);
    }
...
function concatcbTask(params, status, logger) {
    ...
    case State.TYPES.FILES:
        concatString = concatcbFilesSync(callback, destination, this._state.get().value, encoding);
