...
deployment to CDN, bootstraping, etc.) through configuration.
...
For all the options avaliable in Shaker, see `Shaker Configuration API <./shaker_api_summary.html>`_.
...
If there are no high-coverage bundles, the default behaviour is to load the mojits as 
**low coverage**. 
...
but you will have to hardcode the code into your controllers, and this is not really ideal.
...
served when the mojit is dispatched. Given that a mojit can have different behaviour 
...
stylesheets that come after the ``script`` tag don't start downloading 
...
The alternative we suggest is to dynamically load scipts. Here is a snippet illustrating
